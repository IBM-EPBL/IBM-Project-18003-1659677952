...
class Video(object):
	def __init__(self):
		...
		self.model = tf.keras.models.load_model('asl_model.h5') # Execute Local Trained Model
	...
	def get_frame(self):
		...
		copy_img = tf.keras.preprocessing.image.load_img('image.jpg', target_size=(64,64))
		x = tf.keras.preprocessing.image.img_to_array(copy_img)
		x = np.expand_dims(x, axis=0)
		pred = np.argmax(self.model.predict(x), axis=1)
		self.y = pred[0]
		cv2.putText(frame,'The Predicted Alphabet is: '+str(self.index[self.y]),(100,50),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),3)
